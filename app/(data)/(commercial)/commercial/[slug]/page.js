...
async function getAllPosts() {
  const res = await fetch(apiUrl + `/commercial/all`, {next: {revalidate: revalidateInterval }})
  if (!res.ok) {
...
async function getSinglePost(slug) {
  const res = await fetch(apiUrl + `/commercial/all/${slug}`, {next: {revalidate: revalidateInterval }});
  if (!res.ok) {
    return notFound();
  } else {
    return res.json();
