...
import ProjectSingle from "../../../../templates/ProjectSingle.js";
...
async function getSinglePost(slug) {
  const res = await fetch(apiUrl + `/projects/all/${slug}`, {next: {revalidate: revalidateInterval }});
  if (!res.ok) {
    return notFound();
  } else {
    return res.json();
...
export default async function Page({ params: { slug } }) {
  ...
  return (
    <>
      <ProjectSingle pageData={post} projects={projects} />
    </>
  );
}
